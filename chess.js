...
class ChessGame {
    ...
    showPromotionDialog(row, col, color) {
        ...
        if (capturedPieces.length > 0) {
            ...
            // If no captured pieces, offer default promotion to queen
            const queenOption = document.createElement('div');
            queenOption.className = `promotion-piece ${color}`;
            queenOption.innerHTML = this.getPieceSymbol({ type: 'queen', color: color });
            queenOption.dataset.type = 'queen';
            queenOption.addEventListener('click', () => {
                this.completePromotion(row, col, 'queen', color);
                document.body.removeChild(dialog);
            });
            piecesContainer.appendChild(queenOption);
        }
